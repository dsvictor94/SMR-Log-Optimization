...
DISCOVER='export IFACE="$(/sbin/ip route get 10.1.1.0 | awk '"'"'{print $4;exit}'"'"')"; echo IFACE=$IFACE'
eval $DISCOVER
...
for i in ${!pattern[@]}; do
    ...
    SETUP="export STATE_TRANSFER=ch.usi.da.smr.statetransfer.TCPStateTransfer; export APPLICATION_LOGGER=ch.usi.da.smr.log.InMemory; export TCP_STATE_TRANSFER_PORT=5555"
    echo "Starting Replicas"
    for node in "${replica_nodes[@]}"; do
        # need the sleep becouse re replica stops if stdin close
        exec_at --output "$(output replica "$i" "$node")" "$node" sh -c "$DISCOVER; $SETUP; sleep 100000000 | ${smr_folder}/replica.sh 1,$((nodeid++)),0 0 ${zoo}"
    done
    pause
    ...
    echo "Pausing ${half_duration}s to add a replica"
    sleep $half_duration
    ...
    echo "Pausing ${half_duration}s to stop test"
    sleep $half_duration
